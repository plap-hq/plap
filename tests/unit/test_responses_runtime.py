...
def _ingested(*, active: set[str] | None = None, checkpoint_required: bool = False) -> Ingested:
    return Ingested(
        memory={},
        threads=Threads(enabled={"main"} if active is None else active),
        main_tail=None,
...
async def test_state_save_progress_appends_cross_lane_stub_without_main_patch() -> None:
    ...
    state = _state(
        ingested=Ingested(
            memory={},
            threads=Threads(enabled={"main"}, messages={"main": [assistant]}),
...
async def test_state_save_progress_preserves_inactive_cross_lane_call_without_stub() -> None:
    ...
    state = _state(
        ingested=Ingested(
            memory={},
            threads=Threads(enabled=set(), messages={"main": [assistant]}),
            main_tail=HiddenMainTail(source=assistant),
            last_reasoning_id=None,
        )
    )
...
@pytest.mark.parametrize("location", ["enabled", "blocked_by", "messages"])
async def test_state_emission_rejects_unconfigured_threads(operation: str, location: str) -> None:
    state = _state()
    if location == "enabled":
        state.threads.enable("unknown")
    elif location == "blocked_by":
        state.threads.block("unknown", by="test")
    else:
...
async def test_state_commit_emits_full_non_main_checkpoint_after_user() -> None:
    ...
    assert payload.state.enabled == {"main", "reviewer"}
...
async def test_run_response_emits_reactivated_persisted_main_call_without_main_completion() -> None:
    ...
    assert reasoning.state.enabled == {"main"}
...
async def test_state_commit_does_not_republish_active_public_tail() -> None:
    ...
    state = _state(
        ingested=Ingested(
            memory={},
            threads=Threads(enabled={"main"}, messages={"main": [public]}),
...
async def test_state_commit_does_not_republish_reactivated_public_tail() -> None:
    ...
    state = _state(
        ingested=Ingested(
            memory={},
            threads=Threads(enabled=set(), messages={"main": [public]}),
            main_tail=PublicMainTail(source=source),
            last_reasoning_id="rs_public",
        )
    )
...
async def test_state_commit_does_not_publish_reactivated_compacted_tail() -> None:
    ...
    state = _state(
        ingested=Ingested(
            memory={},
            threads=Threads(enabled=set(), messages={"main": [snapshot]}),
